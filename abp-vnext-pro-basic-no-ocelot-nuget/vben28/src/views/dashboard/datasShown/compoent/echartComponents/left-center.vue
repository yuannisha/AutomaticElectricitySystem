<script setup lang="ts">
  ...
  onMounted(() => {
    const getPowerSwitchsNum = setInterval(async () => {
      state.offlineNum = 0;
      state.onlineNum = 0;
      state.isAbnormalNum = 0;
      state.isNotAbnormalNum = 0;
      state.totalNum = 0;
      var powerSwitchs = await GetAllPowerSwitchs();
      console.log(powerSwitchs);
      powerSwitchs.forEach((x) => {
        if (x.isAbnormal == 1) state.isAbnormalNum++;
        else state.isNotAbnormalNum++;
        if (x.isOnline == 1) state.onlineNum++;
        else state.offlineNum++;
      });
      state.totalNum = powerSwitchs.length;
      setOption();
      console.log(state);
    }, 2000);
